...
class MujocoViewer:
    ...
    def __init__(self, model,  dt = 1/60, width=1920, height=1080, start_paused=False,
                 custom_render_callback=None, record=False, camera_params=None,
                 default_camera_mode="follow", hide_menu_on_startup=None,
        ...
        self._loop_count = 0
        self._target_render_time = 1 / 60.
        self._time_per_render = self._target_render_time
    ...
    def render(self, data, record = False):
        """
        ...
        def render_inner_loop(self):
            ...
            time.sleep(max(0, self._target_render_time - (time.time() - render_start) - 0.0002))
            self._time_per_render = time.time() - render_start
